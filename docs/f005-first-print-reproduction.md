...
```sh
git apply <project-root>/patches/klipper/0002-x2000-passive-uart-attach.patch
```
...
```sh
<stock-python> scripts/f005_reset_from_shutdown_once.py \
...
`scripts/f005_phase2_sanitize_gcode.py` on a copy of the slicer output, then
hash the sanitized file and use `scripts/f005_phase2_pty_feeder.py` against the
...
```sh
python3 scripts/f005_phase2_sanitize_gcode.py <slicer-input.gcode> <stage-f.gcode>
sha256sum <stage-f.gcode>
<stock-python> scripts/f005_phase2_pty_feeder.py \
